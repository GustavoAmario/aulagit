...
function Ativos(){
  ...
  const fetchCategoriasList = async () => {
    ...
    try{
      ...
      console.error("Erro ao busca categorias:", error);
    } finally {
      setIsLoading(true);
    }
  };
  ...
  const handleDeleteAtivo = (idAtivo) => {
    ...
    if (confirmDelete){
      ...
      .then((response) => {
        if (response.ok){
          const updatedAtivos = ativos.filter((ativo) => ativo.idAtivo !== idAtivo);
...
function Ambientes(){
  ...
  const handleDeleteAmbiente = (idAmbiente) => {
    ...
    if (confirmDelete){
      ...
      .then((response) => {
        if (response.ok){
          const updatedAmbientes = ambientes.filter((ambiente) => ambiente.idAmbiente !== idAmbiente);
      ...
      {isAddingCategoria || editingCategoria ? (
        ...
        {editingCategoria ? (
          <div>
            ...
            <button className="cancel" onClick={handleCancelEdit}>Cancel Edição</button>
          </div>
        ) : (
